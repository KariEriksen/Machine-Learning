...
As mentioned in the section above we may come across problems where the columns in $X$ are not linear independent. Then the coefficients in $\beta$ are not uniquely defined through least square. One way to solve this is by Ridge regression.
...
\begin{equation}
\mathbf{X}^{\top}\mathbf{X} \ \rightarrow \ \mathbf{X}^{\top}\mathbf{X} + \lambda\mathbf{I}_{pp}
\end{equation}
